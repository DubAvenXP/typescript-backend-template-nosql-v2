...
import { err, success } from "./../../helpers/response";
...
export async function sendEmail(req: any, res: Response) {
    ...
    try {
        ...
        <li style="margin: 1rem 0">
            <span style="font-weight: bold;">Nombre: </span><span style="color: #000">${name}</span>
        </li>
        <li style="margin: 1rem 0">
        ...
        success(req, res, { msg: "Mensaje enviado con exito!" }, 200);
    } catch (error) {
        console.error(error);
